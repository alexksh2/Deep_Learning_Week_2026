...
def chunk_pages(pages: List[Dict[str, Any]], chunk_size: int, chunk_overlap: int) -> List[Dict[str, Any]]:
    """Split pages into overlapping fixed-size chunks with citation metadata."""
    chunks = []
    for page in pages:
        text     = page["text"]
        page_num = page["page"]
...
def evaluate_answer(response: str, citations: List[Dict[str, Any]], chunks: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Use Ollama to judge how well the answer is supported by its sources."""
    if not citations:
        return None
    ...
    for cit in citations[:4]:
        full = lookup.get(cit["id"], {}).get("text", cit["text"])
        cited_sections += f"\n[{cit['id']}] Page {cit['page']}: {full[:400]}\n"

    prompt = f"""Evaluate whether the following answer is well-supported by the cited source sections.
...
- "assessment": one of "well_supported" | "partially_supported" | "unsupported"
- "reasoning": one concise sentence

Output ONLY valid JSON, no markdown."""

    try:
        raw = call_ollama(prompt, max_tokens=256)
        raw = re.sub(r"^```(?:json)?\s*", "", raw, flags=re.IGNORECASE)
        raw = re.sub(r"\s*```\s*$", "", raw)
        return json.loads(raw)
    except Exception as e:
        return {"confidence": None, "assessment": "evaluation_failed", "reasoning": str(e)}
